...
addTodoBtn.addEventListener("click", (event) => {
  ...
  // Get the todo text, date, description, and priority
  const todoText = document.getElementById("todo-text").value;
  const todoDate = document.getElementById("todo-date").value;
  const todoDescription = document.getElementById("todo-description").value;
  const todoPriority = document.getElementById("todo-priority").value; // Get priority

  // If the text, date, and description are not empty
  if (todoText && todoDate && todoDescription && todoPriority !== 'Select Priority') {
    // Create a new todo object
    const todo = {
      text: todoText,
      date: todoDate,
      description: todoDescription,
      priority: todoPriority, // Add priority to the todo object
      state: "pending",
      id: new Date().getTime(),
    };
    ...
    }, 1000); // Message will be shown for 1 second
    ...
    document.getElementById("todo-priority").value = "Select Priority";
  } else if (!todoText) {
  ...
  } else if (!todoPriority === 'Select Priority') {
...
const loadTodos = (filter, filteredTodos) => {
  ...
  const todoList = document.getElementById("todos-table");
  todoList.innerHTML = "";
...
const createTodoElement = (todo) => {
  ...
  // Set priority color class based on priority value
  const priorityClass = {
    low: "low",
    medium: "medium",
    high: "high",
  ...
  return `
    <div class="todo">
      <td><button class="todo-btn"><i class="fa-regular regular ${todoButtonIconClass}"></i></button></td>
      <td>${capitalizeFirstLetter(todo.text)}</td>
      <td>${capitalizeFirstLetter(todo.description)}</td>
      <td><span class="priority ${priorityClass}">${
    todo.priority
  }</span></td> <!-- Priority with color -->
      <td><button class="delete-btn"><i class="fa-solid fa-trash delet-btn"></i></button></td>
      <td><span class="${todoDateClass}">${formatDate(todo.date)}</span></td>
    </div>
  `;
};
...
document.body.addEventListener("click", (event) => {
  if (event.target.closest(".todo-btn")) {
    const todoItem = event.target.closest("tr");
